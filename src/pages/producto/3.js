...
export default function ProductDetail3() {
  ...
  const [carritoConProductos, setCarritoConProductos] = useState(false);
      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 px-6">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-[var(--color-primary)] mt-4">Precio unitario: {product.precio}</p>
            <p className="text-xl font-semibold mt-4">Cantidad: {quantity}</p>

            {/* Cantidad */}
            <div className="mt-6 flex items-center gap-2">
              <button onClick={() => handleQuantityChange("decrement")} className="p-2 bg-gray-300 rounded-md">-</button>
              <input
                type="number"
                value={quantity}
                min="1"
                ...
                readOnly
              />
              <button onClick={() => handleQuantityChange("increment")} className="p-2 bg-gray-300 rounded-md">+</button>
            </div>
