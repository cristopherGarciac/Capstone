...
import { Prisma } from "@prisma/client"; // Necesario para Prisma.Decimal

// Convertir Prisma.Decimal a number (profundo)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "PUT") {
      ...
      if (data.sku) {
        const colision = await prisma.productos.findUnique({
          where: { sku: data.sku },
        });
    ...
    if (err?.code === "P2002")
      return res.status(409).json({ error: "SKU duplicado", meta: err.meta });
